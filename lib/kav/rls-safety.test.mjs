...
const migrationSource = [
  ...
  "20260902090000_team_shared_equipment.sql",
  "20260907101533_manager_activity_events.sql",
  "20260908214845_leave_request_counts_and_email_normalization.sql",
  "20260908221242_leave_request_day_counts.sql",
  "20260909160742_scope_leave_request_day_counts_to_active_requests.sql",
...
test("the leave request day-count RPC is period-scoped and excludes cancelled or rejected requests", () => {
  const definition = [...migrationSource.matchAll(
    /create or replace function private\.get_team_leave_request_day_counts\([\s\S]*?\$\$;/g,
  )].map((match) => match[0]).at(-1);
  assert.match(definition, /l\.reserve_period_id = target_reserve_period_id/);
  assert.match(definition, /l\.status not in \('cancelled', 'rejected'\)/);
});
...
test("team shared equipment has scoped RLS and manager-only writes", () => {
  assert.match(migrationSource, /create table if not exists public\.team_equipment_items/);
  assert.match(migrationSource, /create table if not exists public\.team_equipment_transfers/);
  assert.match(migrationSource, /alter table public\.team_equipment_items enable row level security/);
  assert.match(migrationSource, /alter table public\.team_equipment_transfers enable row level security/);
  assert.match(migrationSource, /create or replace function private\.can_view_team_equipment_item/);
  assert.match(migrationSource, /tm\.role in \('admin', 'manager'\)/);
  assert.match(migrationSource, /p\.id in \(target_permanent_owner_person_id, target_current_holder_person_id\)/);
  assert.match(migrationSource, /team_equipment_items_insert[\s\S]*private\.can_manage_team\(team_id\)/);
  assert.match(migrationSource, /team_equipment_items_update[\s\S]*private\.can_manage_team\(team_id\)/);
  assert.match(migrationSource, /team_equipment_transfers_insert[\s\S]*private\.can_manage_team\(team_id\)/);
});

test("activity events are manager-readable and team-member inserted", () => {
  assert.match(migrationSource, /create table if not exists public\.activity_events/);
  assert.match(migrationSource, /alter table public\.activity_events enable row level security/);
  assert.match(migrationSource, /activity_events_select_manage[\s\S]*private\.can_manage_team\(team_id\)/);
  assert.match(migrationSource, /activity_events_insert_team_member[\s\S]*actor_user_id = \(select auth\.uid\(\)\)/);
  assert.match(migrationSource, /from public\.team_memberships tm[\s\S]*tm\.is_active/);
});
