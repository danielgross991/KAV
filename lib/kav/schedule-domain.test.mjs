...
test("schedule default keeps operational period semantics first", () => {
  assert.equal(selectDefaultScheduleReservePeriod([
    { id: "next", status: "published", starts_on: "2026-10-01", ends_on: "2026-10-31" },
    activePeriod,
  ], operationalDate)?.id, "active");
});

test("schedule default falls forward to the next visible period", () => {
  assert.equal(selectDefaultScheduleReservePeriod([
    { id: "past", status: "completed", starts_on: "2025-05-01", ends_on: "2025-06-01" },
    { id: "next", status: "draft", starts_on: "2026-09-06", ends_on: "2026-12-02" },
  ], "2026-08-30")?.id, "next");
});

test("schedule default falls back to latest completed period when no future line exists", () => {
  assert.equal(selectDefaultScheduleReservePeriod([
    { id: "older", status: "completed", starts_on: "2025-01-01", ends_on: "2025-02-01" },
    { id: "latest", status: "completed", starts_on: "2025-05-28", ends_on: "2025-08-07" },
  ], operationalDate)?.id, "latest");
});
...
const operational = (date, options = {}) => resolveOperationalPerson({
  personId: "p", date, memberships, blocks, overrides: options.overrides ?? [],
  leaves: options.leaves ?? [], attendanceEntries: options.attendanceEntries ?? [],
});
const leave = (status, approvedStartsOn = "2026-09-03", approvedEndsOn = "2026-09-03") => ({
  id: "leave", personId: "p", status, startsOn: "2026-09-02", endsOn: "2026-09-05",
  approvedStartsOn, approvedEndsOn,
});

test("operational resolver keeps expected base unreported without exceptions", () => {
  const result = operational("2026-09-03");
  assert.equal(result.expectedAtBase, true);
  assert.equal(result.attendance, "unreported");
  assert.equal(result.discrepancy, "unreported");
});

test("approved leave removes expectation while pending leave does not", () => {
  assert.equal(operational("2026-09-03", { leaves: [leave("approved")] }).expectedAtBase, false);
  assert.equal(operational("2026-09-03", { leaves: [leave("pending")] }).expectedAtBase, true);
});

test("approved leave and actual presence remain separate layers", () => {
  const result = operational("2026-09-03", {
    leaves: [leave("approved")], attendanceEntries: [{ personId: "p", isPresent: true }],
  });
  assert.ok(result.leave);
  assert.equal(result.attendance, "present");
  assert.equal(result.discrepancy, "unexpected-presence");
});

test("expected absence is an attendance discrepancy", () => {
  const result = operational("2026-09-03", { attendanceEntries: [{ personId: "p", isPresent: false }] });
  assert.equal(result.expectedAtBase, true);
  assert.equal(result.discrepancy, "attendance-gap");
});

test("home presence is unexpected", () => {
  const result = operational("2026-09-10", { attendanceEntries: [{ personId: "p", isPresent: true }] });
  assert.equal(result.expectedAtBase, false);
  assert.equal(result.discrepancy, "unexpected-presence");
});

test("rotation overrides change effective expectation in both directions", () => {
  const homeToBase = resolveOperationalPerson({
    personId: "p", date: "2026-09-03",
    memberships: [{ personId: "p", groupId: "b", startsOn: "2026-09-01", endsOn: "2026-09-30" }],
    blocks: [
      ...blocks,
      { groupId: "b", state: "home", startsOn: "2026-09-01", endsOn: "2026-09-07" },
    ],
    overrides: [{ personId: "p", fromGroupId: "b", toGroupId: "a", startsOn: "2026-09-03", endsOn: "2026-09-03" }],
    leaves: [], attendanceEntries: [],
  });
  const baseToHome = resolveOperationalPerson({
    personId: "p", date: "2026-09-03", memberships, blocks,
    overrides: [{ personId: "p", fromGroupId: "a", toGroupId: "b", startsOn: "2026-09-03", endsOn: "2026-09-03" }],
    leaves: [], attendanceEntries: [],
  });
  assert.equal(homeToBase.expectedAtBase, true);
  assert.equal(baseToHome.expectedAtBase, false);
});

test("partial leave affects only its approved dates", () => {
  const partial = leave("partially_approved", "2026-09-11", "2026-09-13");
  partial.startsOn = "2026-09-10"; partial.endsOn = "2026-09-14";
  for (const date of ["2026-09-10", "2026-09-14"]) assert.equal(operational(date, { leaves: [partial] }).leave, null);
  for (const date of ["2026-09-11", "2026-09-12", "2026-09-13"]) assert.ok(operational(date, { leaves: [partial] }).leave);
});

test("leave validation rejects approved dates outside request", () => {
  assert.ok(validateLeaveRange({
    requested: { startsOn: "2026-09-10", endsOn: "2026-09-14" },
    approved: { startsOn: "2026-09-09", endsOn: "2026-09-13" },
    period: { startsOn: "2026-09-01", endsOn: "2026-09-30" },
  }).includes("approved-outside-requested"));
});

test("duplicate attendance for a person is rejected by the resolver", () => {
  assert.throws(() => operational("2026-09-03", { attendanceEntries: [
    { personId: "p", isPresent: true }, { personId: "p", isPresent: false },
