...
test("a planned home rotation day does not count toward the home leaderboard without actual data", () => {
  const resolutions = new Map([
    ["p1", [
      { attendance: "present", expectedAtBase: true, leave: false, state: "base" },
  ...
  const [stats] = computeAttendanceStats([people[0]], resolutions);
  assert.equal(stats.homeDays, 0);
  assert.equal(stats.finalizedExpectedDays, 1);
  assert.equal(stats.attendancePercentage, 1);
});

test("base and home counts use actual attendance instead of planned rotation state", () => {
  const resolutions = new Map([
    ["p1", [
      { attendance: "present", expectedAtBase: true, leave: false, state: "base" },
      { attendance: "present", expectedAtBase: false, leave: false, state: "home" },
      { attendance: "absent", expectedAtBase: true, leave: false, state: "base" },
      { attendance: "unreported", expectedAtBase: true, leave: false, state: "base" },
    ]],
  ]);
  const [stats] = computeAttendanceStats([people[0]], resolutions);
  assert.equal(stats.baseDays, 2);
  assert.equal(stats.homeDays, 1);
  assert.equal(stats.totalElapsedDays, 3);
  assert.equal(stats.homePercentage, 1 / 3);
});
...
test("an actual absence on the first base day counts toward the home leaderboard", () => {
  const resolutions = new Map([
    ["p1", [
      { attendance: "present", expectedAtBase: true, leave: false, state: "base" },
    ]],
  ...
  assert.equal(stats.find((item) => item.personId === "p1")?.homeDays, 0);
  assert.equal(stats.find((item) => item.personId === "p2")?.homeDays, 1);
  assert.equal(ranked[0].personId, "p2");
});

test("home leaderboard is based on actual absences instead of planned home state", () => {
  const resolutions = new Map([
    ["p1", [
      { attendance: "absent", expectedAtBase: true, leave: false, state: "base" },
    ]],
  ...
  const stats = computeAttendanceStats(people, resolutions);
  const ranked = rankHomeLeaderboard(stats);
  assert.equal(stats.find((item) => item.personId === "p1")?.homeDays, 1);
  assert.equal(stats.find((item) => item.personId === "p2")?.homeDays, 0);
  assert.equal(ranked[0].personId, "p1");
});
...
test("home leaderboard matches mixed actual Otniel attendance snapshots", () => {
  const roster = [
    { fullName: "אורי בנבג'י", id: "uri" },
    { fullName: "אביאל אלקאיל", id: "aviel" },
    { fullName: "בנימין ברי", id: "beri" },
    { fullName: "גיא כהן", id: "guy" },
  ];
  const resolutions = new Map([
    ["uri", [
      { attendance: "present", expectedAtBase: true, leave: false, state: "base" },
  ...
  const stats = computeAttendanceStats(roster, resolutions);
  const byName = Object.fromEntries(stats.map((item) => [item.fullName, item]));
  assert.equal(byName["אורי בנבג'י"].baseDays, 3);
  assert.equal(byName["אביאל אלקאיל"].baseDays, 2);
  assert.equal(byName["בנימין ברי"].baseDays, 0);
  assert.equal(byName["גיא כהן"].baseDays, 1);
  assert.deepEqual(rankHomeLeaderboard(stats).map((item) => item.fullName), [
    "בנימין ברי",
    "גיא כהן",
    "אביאל אלקאיל",
    "אורי בנבג'י",
