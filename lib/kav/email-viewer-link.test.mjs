...
const emailNormalizationMigrationSource = readFileSync(
  join(root, "supabase", "migrations", "20260908214845_leave_request_counts_and_email_normalization.sql"),
  "utf-8",
);
...
test("viewer email login creates and verifies a server-side magic link without emailing the user", () => {
  ...
  assert.match(loginActionsSource, /person\.email\?\.trim\(\)\.toLowerCase\(\) === email/);
});
...
test("person emails are normalized before viewer login matching", () => {
  assert.match(emailNormalizationMigrationSource, /update public\.people[\s\S]*lower\(trim\(email\)\)/);
  assert.match(emailNormalizationMigrationSource, /people_email_lowercase_check/);
  assert.match(usersActionSource, /required\(formData, "email"\)\.toLowerCase\(\)/);
});
