...
test("schedule day tap opens an in-page day preview before full detail", () => {
  ...
  assert.match(scheduleViewSource, /teamLeaveCount \? <span className="kav-num min-w-5/);
...
test("dashboard surfaces manager leave requests and request champions", () => {
  assert.match(dashboardSource, /getLeaveRequestCounts/);
  assert.match(dashboardSource, /managerLeaveRequests:/);
  assert.match(dashboardSource, /leaveRequestLeaderboard:/);
  assert.match(managerLeaveSource, /\.from\("leave_requests"\)/);
  assert.match(dashboardViewSource, /function ManagerLeaveRequests/);
  assert.match(dashboardViewSource, /function LeaveRequestLeaderboard/);
  assert.match(dashboardViewSource, /אלופי בקשות יציאה/);
  assert.match(dashboardViewSource, /כל הבקשות בקו הנבחר/);
});
...
test("leave requests are single-surface for managers and approval is yes or no", () => {
  ...
  assert.match(leavePageSource, /const statusOptions = \[\["pending", "טרם הוחלט"\], \["approved", "כן"\], \["rejected", "לא"\]\]/);
  assert.match(leaveActionsSource, /const STATUSES = \["pending", "approved", "rejected"\]/);
...
test("loading overlays clear after completed navigation or stale pending state", () => {
  ...
  assert.match(formPendingOverlaySource, /return expired \? null : <KavLoading/);
});
