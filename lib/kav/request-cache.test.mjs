...
test("home champion stats include only whole-line operational dates", () => {
  assert.match(statsSource, /wholeLineDates/);
  assert.match(statsSource, /activePeople\.every\(\(person\) => range\.resolve\(person\.id, date\)\.state !== null\)/);
  assert.match(statsSource, /wholeLineDates\.map/);
});
