...
const globalsSource = readFileSync(join(__dirname, "..", "..", "app", "globals.css"), "utf-8");
...
const dailyQuoteCardSource = readFileSync(join(__dirname, "..", "..", "components", "daily-quote-card.tsx"), "utf-8");
const dailyQuotesSource = readFileSync(join(__dirname, "daily-quotes.ts"), "utf-8");
...
const statsSource = readFileSync(join(__dirname, "stats.ts"), "utf-8");
...
test("home champion stats ignore open draft attendance days", () => {
  assert.match(statsSource, /get_team_attendance_day_status/);
  assert.match(statsSource, /status === "submitted"/);
  assert.match(statsSource, /submittedAttendanceDates\.has\(date\) \? resolution\.attendance : "unreported"/);
});

test("home champion stats include commander-only days for people who served them", () => {
  assert.match(statsSource, /elapsedDates\.map/);
  assert.doesNotMatch(statsSource, /wholeLineDates/);
});

test("manager dashboard shows all home champion percentages below the podium", () => {
  assert.match(dashboardViewSource, /function HomeLeaderboardFullStats/);
  assert.match(dashboardViewSource, /אחוזי כל הצוות/);
  assert.match(dashboardViewSource, /data\.canManage \? <HomeLeaderboardFullStats/);
  assert.match(dashboardViewSource, /Math\.round\(item\.homePercentage \* 100\)\}%/);
});

test("dashboard daily quote displays the submitting person when available", () => {
  assert.match(dailyQuotesSource, /submitted_person_id/);
  assert.match(dailyQuotesSource, /submitterName/);
  assert.match(dailyQuotesSource, /submitterPhotoUrl/);
  assert.match(dailyQuotesSource, /full_name, photo_url/);
  assert.match(dailyQuotesSource, /\.from\("people"\)/);
  assert.match(dailyQuoteCardSource, /function SubmitterAvatar/);
  assert.match(dailyQuoteCardSource, /הוגש ע״י/);
  assert.match(dailyQuoteCardSource, /quote\.submitterName/);
  assert.match(dailyQuoteCardSource, /quote\.submitterPhotoUrl/);
});
...
test("team shell shows the current person profile entry point and welcome state", () => {
  ...
  assert.match(appShellSource, /overflow-x-clip/);
  assert.match(globalsSource, /html \{[\s\S]*overflow-x: hidden/);
  assert.match(globalsSource, /body \{[\s\S]*overflow-x: hidden/);
  assert.match(globalsSource, /\.kav-page \{[\s\S]*max-width: min\(1120px, 100%\)/);
  ...
  assert.match(userProfileMenuSource, /closeOnOutsidePress/);
  assert.match(userProfileMenuSource, /event\.key === "Escape"/);
});
...
test("loading overlays clear after completed navigation or stale pending state", () => {
  ...
  assert.match(formPendingOverlaySource, /const pendingOverlayDelayMs = 120/);
  assert.match(formPendingOverlaySource, /const maxPendingOverlayMs = 12000/);
