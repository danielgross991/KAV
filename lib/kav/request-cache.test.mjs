...
test("manager leave requests can be filtered by person and highlight risky dates first", () => {
  ...
  assert.match(leavePageSource, /ברירת מחדל: ממויין לפי תאריכים/);
  ...
  assert.match(leavePageSource, /eachCalendarDate\(leave\.starts_on, leave\.ends_on\)/);
  assert.match(leavePageSource, /leave\.status === "cancelled" \|\| leave\.status === "rejected"/);
  assert.match(leavePageSource, /\.filter\(\(day\) => day\.count > 3\)/);
});
